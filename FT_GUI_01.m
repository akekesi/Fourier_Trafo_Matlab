function varargout = FT_GUI_01(varargin)
% FT_GUI_01 MATLAB code for FT_GUI_01.fig
%      FT_GUI_01, by itself, creates a new FT_GUI_01 or raises the existing
%      singleton*.
%
%      H = FT_GUI_01 returns the handle to a new FT_GUI_01 or the handle to
%      the existing singleton*.
%
%      FT_GUI_01('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FT_GUI_01.M with the given input arguments.
%
%      FT_GUI_01('Property','Value',...) creates a new FT_GUI_01 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FT_GUI_01_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FT_GUI_01_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FT_GUI_01

% Last Modified by GUIDE v2.5 05-Jul-2020 03:14:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FT_GUI_01_OpeningFcn, ...
                   'gui_OutputFcn',  @FT_GUI_01_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FT_GUI_01 is made visible.
function FT_GUI_01_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FT_GUI_01 (see VARARGIN)
...
% Choose default command line output for FT_GUI_01
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes FT_GUI_01 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FT_GUI_01_OutputFcn(hObject, eventdata, handles) 
...
% Fourier-Trafo
[a,b,~,~,~,F] = FT_GUI_Fourier_Trafo_Value(Funk,nft,x);
...
while gstop == 0 && isempty(findobj(handles.figure1)) == 0
    for n = 1:1:length(x)
        ...
        p_funk = plot(ax1,x(1,1:N),Funk(1,1:N),'Color','#77AC30');
        hold(ax1,'on')
        ...
        if Funk(1,n) < F(1,n,end)
            col = '#D95319';
        else
            col = '#0072BD';
        end
        plot(ax1,[x(n) x(n)],[Funk(1,n) F(1,n,end)],'Color',col)
        plot(ax1,x(1,n),Funk(1,n),'Marker','.','MarkerSize',10,'Color','#77AC30');       
