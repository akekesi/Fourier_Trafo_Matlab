...
function [a,b,S,C,CS,F] = FT_GUI_Fourier_Trafo_Value(funk,nft,x)
T = x(end)-x(1);
a = zeros(1,nft);
b = a;
...
n = 2;
for k = 1:1:nft
    a(1,k) = 2/T*FT_GUI_Integral_Trapez_Value(x,funk.*c(x,k));
    if abs(a(1,k)) < 10^-12
        a(1,k) = 0;
    end

    b(1,k) = 2/T*FT_GUI_Integral_Trapez_Value(x,funk.*s(x,k));
    if abs(b(1,k)) < 10^-12
        b(1,k) = 0;
    end
    ...
    CS(n,:) = CS(n-1,:)+C(1,:,k);               % y-Werte zummanaddiert nacheinander
    n = n+1;
    CS(n,:) = CS(n-1,:)+S(1,:,k);               % y-Werte zummanaddiert nacheinander
    n = n+1;
